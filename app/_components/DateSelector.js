...
function DateSelector({ settings, cabin, bookedDates }) {
  ...
  const areSameDay = range.from && range.to && isSameDay(range.from, range.to);
  const numNights = range.from && range.to && !areSameDay ? differenceInDays(range.to, range.from) : 0;
